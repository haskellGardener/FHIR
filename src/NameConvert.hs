...
import Text.XML.HaXml.Schema.NameConversion hiding (escape)
import Data.Char
import Data.List

-- | A simple default set of rules for resolving XNames into HNames.
fhirNameConverter :: NameConverter
fhirNameConverter = NameConverter
    { modid    = \(XName qn)-> HName . mkConid . hierarchy                      $ qn
    , conid    = \(XName qn)-> HName . mkConid . hierarchy                      $ qn
    , varid    = \(XName qn)-> HName . mkVarid . last avoidKeywords . hierarchy $ qn
    , unqconid = \(XName qn)-> HName . mkConid . local                          $ qn
    , unqvarid = \(XName qn)-> HName . mkVarid . last avoidKeywords . local     $ qn
    , fwdconid = \(XName qn)-> HName . ("Fwd" ++) . mkConid . local             $ qn
    , fieldid  = \(XName qnt) (XName qnf) -> HName $ (mkVarid . last id . hierarchy $ qnt)
                                                     ++ "_" ++
  ...
  where
    ...
    mkConid  [c]        | map toLower c == "string"     = "Xsd.XsdString"
                        | otherwise = first toUpper $ concatMap escape c
    mkConid [m,c]       | map toLower c == "string"     = "Xsd.XsdString"
                        ...
                        | otherwise = first toUpper m ++ "." ++ first toUpper (concatMap escape c)
    mkConid more        = mkConid [concat more]
    mkVarid  [v]        = first toLower $ concatMap escape v
    mkVarid [m,v]       = first toUpper m ++ "." ++ first toLower (concatMap escape v)
